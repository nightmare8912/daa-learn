...
public:
    ...
    void prims(int src = 0)
    {
        ...
        {   
            ...
            {
                if (!q.find(temp->data.to)->data.isProcessed && temp->data.weight < q.find(temp->data.to)->data.distance)
                {
                    v[temp->data.to].pred = q.find(temp->data.to)->data.pred = currVert->data.data;
                    v[temp->data.to].distance = q.find(temp->data.to)->data.distance = temp->data.weight;
                }
            }
        }
        printRes();       
